"""
title: OpenAI GPT Image 2 (Action)
author: open-webui
date: 2026-04-21
version: 1.2
required_open_webui_version: 0.5.0
license: MIT
description: An action button for toggled image generation and editing with OpenAI GPT Image 2 using conversation-aware reference images, sticky output settings, and Open WebUI image persistence.
...
class Action:
    class Valves(BaseModel):
        priority: int = Field(
            default=0,
            description="Controls button display order. Lower values appear first.",
        )
        require_confirmation: bool = Field(
            default=False,
            description="Ask for confirmation before generating an image.",
        )
    ...
    def __init__(self):
        self.id = "gpt_image_2_action"
        self.name = "Generate Image"
        self.valves = self.Valves()
        self.last_emit_time = 0.0

    async def on_startup(self):
        print("[gpt_image_2_action] Action loaded successfully - version 1.0")
        print("[gpt_image_2_action] Action type: Button-triggered image generation")
        print("[gpt_image_2_action] Features: generation, multi-image references, mask edits, sticky settings")
        print(f"[gpt_image_2_action] Debug mode: {self.valves.debug}")
    ...
    def _debug(self, msg: str) -> None:
        if self.valves.debug:
            print(f"[gpt_image_2_action][DEBUG] {msg}")
    ...
    async def action(
        ...
        if not api_key:
            error_msg = "❌ Error: OpenAI API key not configured. Please set your API key in the action settings."
            await self.emit_status(__event_emitter__, "error", error_msg, True)
            return self._action_response(body, error_msg)

        try:
            body = self._normalize_action_body(body)
            ...
            if not prompt:
                if self._is_system_followup(messages):
                    ...
                    return self._action_response(body, "")

                error_msg = "❌ Error: No prompt provided for image generation."
                return self._action_response(body, error_msg)

            if self.valves.require_confirmation and __event_call__:
                confirmed = await __event_call__(
                    {
                        "type": "confirmation",
                        "data": {
                            "title": "Generate Image",
                            "message": f"Generate an image with GPT Image 2 using this prompt?\n\n{prompt[:500]}",
                        },
                    }
                )
                if not confirmed:
                    cancelled_msg = "Image generation cancelled."
                    await self.emit_status(__event_emitter__, "info", cancelled_msg, True)
                    return self._action_response(body, cancelled_msg)
            ...
            return self._action_response(body, response_content)
        except Exception as e:
...
{{MESSAGES:END:6}}"""
    ...
    def _extract_follow_ups_from_text(self, text: str) -> List[str]:
        ...
        try:
            ...
            if isinstance(parsed, dict) and isinstance(parsed.get("follow_ups"), list):
                return [str(item) for item in parsed["follow_ups"] if str(item).strip()][:5]
        except Exception:
            pass
    ...
    def _normalize_action_body(self, body: dict) -> dict:
    ...
    def _action_response(self, body: dict, content: str) -> dict:
