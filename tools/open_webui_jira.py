...
class IssueFormatter:
    ...
    @staticmethod
    def format_issue_details_html(issue: Dict[str, Any], comments: List[Dict[str, Any]] = None) -> str:
        """Generate rich HTML for issue details with Jira-style formatting"""
        
        # Status color mapping
        status_colors = {
            "done": "#00875A", "closed": "#00875A", "resolved": "#00875A",
            "in progress": "#0052CC", "in review": "#0052CC",
            "backlog": "#6554C0", "to do": "#6554C0",
        }
        status_color = status_colors.get(issue["status"].lower(), "#5E6C84")
        
        # Priority color mapping
        priority_colors = {
            "highest": "#DE350B", "high": "#FF5630",
            "medium": "#FF991F", "low": "#36B37E", "lowest": "#00875A"
        }
        ...
        comments_html = ""
        if comments:
            comments_html = '<h3 style="margin-top: 30px;">💬 Comments (' + str(len(comments)) + ')</h3>'
            for comment in comments:
                ...
                <div class="comment">
                    ...
                    <div class="comment-text">''' + html.escape(text) + '''</div>
                </div>
                '''
...
'''
    ...
    @staticmethod
    def format_issue_list_html(issues: List[Dict[str, Any]], total: Optional[int], displayed: int, start_at: int = 0) -> str:
        ...
        status_colors = {
            "done": "#00875A", "closed": "#00875A", "resolved": "#00875A",
            "in progress": "#0052CC", "in review": "#0052CC",
            "backlog": "#6554C0", "to do": "#6554C0", "ready": "#0052CC",
        }
        
        # Priority color mapping
        priority_colors = {
            "highest": "#DE350B", "high": "#FF5630",
            "medium": "#FF991F", "low": "#36B37E", "lowest": "#00875A", "none": "#5E6C84"
        }
        ...
        pagination_html = ""
        if total is None:
            pagination_html = f"Showing results {start_at + 1}-{current_end}. More may be available."
        elif total > displayed:
            pagination_html = f"Showing results {start_at + 1}-{current_end} of {total} total issues."
        else:
...
class Jira:
    ...
    def search(self, query: str, max_results: int = 50, start_at: int = 0):
        ...
        self.logger.debug(f"Searching issues with query: {query} (max={max_results}, start={start_at})")
        ...
        self.logger.debug(f"Extracted total: {total}, displayed: {len(issues)}, isLast: {is_last}")
    ...
    def create_issue(
        ...
        try:
            ...
            if "400" in str(e):
                self.logger.debug("ADF format failed, retrying with plain text description")
    ...
    def update_issue(
        ...
        if description is not None:
            ...
            self.logger.debug(f"Updating description ({len(description)} chars) using ADF format")
        ...
        self.logger.debug(f"Successfully updated {', '.join(updated_fields)} for {issue_id}")
...
class Tools:
    ...
    class Valves(BaseModel):
        ...
        password: EncryptedStr = Field(
            default="", description="Your Jira API Token (generate at id.atlassian.com/manage-profile/security/api-tokens) or leave empty to use PAT field"
        )
    ...
    def _get_jira_client(self):
        ...
        if self.valves.debug_mode:
            ...
            self.logger.debug(f"PAT provided: {bool(decrypted_pat)}, PAT length: {len(decrypted_pat) if decrypted_pat else 0}")
            self.logger.debug(f"Password provided: {bool(decrypted_password)}, Password length: {len(decrypted_password) if decrypted_password else 0}")
        
        if not decrypted_pat and (
            not self.valves.username or not decrypted_password
        ):
            raise ValueError(
...
"""
            ...
            if description:
                confirmation += f"- **Description**: Updated ({len(description)} characters)\n"
            if priority:
            ...
            await event_emitter.emit_message(confirmation)
            await event_emitter.emit_status(
                f"Successfully updated {issue_id}", True
            )
