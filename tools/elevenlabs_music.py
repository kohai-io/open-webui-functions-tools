...
class Tools:
    """ElevenLabs Music Tool: compose music from prompt and return a downloadable link."""
    ...
    def __init__(self):
        ...
        self.log = logging.getLogger("elevenlabs_music_tool")
    ...
    async def compose_music(
        ...
        prompt: Optional[str] = Field(
            default=None,
            description="Describe the music to generate (genre, mood, instruments, etc.)",
        ),
        music_length_ms: Optional[int] = Field(
            default=None, description="Desired music length in milliseconds (e.g., 10000)"
        ),
        ...
        if __event_emitter__:
            await __event_emitter__(
                {"type": "status", "data": {"description": "Composing music…", "done": False}}
            )

        try:
            audio_bytes, ext, content_type = await self._call_elevenlabs_music(
                api_base, api_key, prompt, music_length_ms, fmt
            )
            if not audio_bytes:
                if __event_emitter__:
                    await __event_emitter__(
                ...
                return "Tell the user that music generation failed"

            file_id = await self._save_file(audio_bytes, prompt or "music", fmt, ext, content_type, __user__)
            ...
            if file_id:
                return (
                    f"✅ Music composed! [Download here](/api/v1/files/{file_id}/content) "
                    f"({size_mb:.1f}MB)"
                )
            else:
                return f"✅ Music composed but failed to save. Size: {size_mb:.1f}MB"
    ...
    async def _call_elevenlabs_music(
        self,
        api_base: str,
        api_key: str,
        prompt: Optional[str],
        music_length_ms: Optional[int],
        output_format: str,
    ) -> tuple[Optional[bytes], str, str]:
        """
        Two modes from docs:
        - POST {api_base}/music (basic)
        - POST {api_base}/music/detailed with {prompt, music_length_ms}
        Both accept output_format as query param, and return JSON with base64 audio or URL.
        """
        use_detailed = bool(prompt) or isinstance(music_length_ms, int)
        endpoint = "/music/detailed" if use_detailed else "/music"
        ...
        if use_detailed:
            if prompt:
                body["prompt"] = prompt
            if isinstance(music_length_ms, int) and music_length_ms > 0:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(url, headers=headers, json=body, params=params) as resp:
                if resp.status != 200:
                    self.log.error(f"ElevenLabs Music error {resp.status}: {(await resp.text())[:300]}")
                    return None, "", ""
                ...
                try:
                    ...
                    self.log.error(f"Parse music response failed: {e}")
                    return None, "", ""
    ...
    async def _save_file(
        ...
        safe_prompt = "".join(c for c in (prompt or "music")[:40] if c.isalnum() or c in (" ", "-", "_")).rstrip().replace(" ", "_")
        timestamp = int(time.time())
        filename = f"music_{safe_prompt}_{timestamp}{ext}"
        ...
        try:
            with open(tmp_path, "rb") as f:
                file_data, file_path = Storage.upload_file(
                    f, filename, {"content_type": content_type, "source": "elevenlabs_music_tool"}
                )
