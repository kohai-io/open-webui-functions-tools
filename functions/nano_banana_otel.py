"""
title: Google Gemini 2.x Flash Image Generation (Chat Pipeline)
author: open-webui
date: 2024-05-30
version: 1.3
license: MIT
description: A pipeline for iterative image generation using Google Gemini with conversation history
requirements: google-genai>=1.41.0, cryptography, requests, google-auth, opentelemetry-api
"""
...
def instrument_with_user_data(func):
    @functools.wraps(func)
    async def wrapper(
        self, body: dict, __user__: Optional[dict] = None, *args, **kwargs
    ):
        tracer = trace.get_tracer(__name__)
        with tracer.start_as_current_span("nano_banana_pipe") as span:
            if __user__:
                user_id = str(__user__.get("id", "unknown"))
                user_email = str(__user__.get("email", "unknown"))
                user_name = str(__user__.get("name", "unknown"))
                user_role = str(__user__.get("role", "unknown"))
                ...
                # Debug logging
                msg = f"OTEL Trace Attributes Set - User: {user_email} ({user_role}), ID: {user_id}"

                # 1. Log to OpenWebUI logger (via self._debug or print)
                if hasattr(self, "_debug"):
                    self._debug(msg)
                else:
                    print(f"[DEBUG] {msg}")

                # 2. Show in OpenWebUI chat session (via emit_status)
                event_emitter = kwargs.get("__event_emitter__")
                if event_emitter and hasattr(self, "emit_status"):
                    try:
                        await self.emit_status(event_emitter, "info", msg, False)
                    except Exception as e:
                        print(f"[DEBUG] Failed to emit OTEL status: {e}")

            return await func(self, body, __user__=__user__, *args, **kwargs)
...
class Pipe:
    class Valves(BaseModel):
        ...
        model_name: str = Field(
            ...
            description="The Google AI model name for image+text generation",
        )
        max_history_images: int = Field(
            default=5,
            description="Maximum number of previous images to include in context",
        )
        ...
        edit_mode: bool = Field(
            default=True,
            description="When true, prioritize editing the last generated image instead of recreating.",
        )
        ...
        aspect_ratio: str = Field(
            default="",
            description="Optional output aspect ratio. Supported: '1:1', '3:4', '4:3', '9:16', '16:9'. If empty, model default is used.",
        )
    ...
    async def pipe(
        ...
        - Always append a single assistant message and return body to avoid duplicate rendering in Open WebUI.
        """
        ...
        try:
            ...
            # Build config with optional aspect ratio per Gemini docs
            # Use proper casing per docs: ['Image', 'Text']
            _cfg_kwargs: dict = {"response_modalities": ["Image", "Text"]}
            _ar = getattr(self.valves, "aspect_ratio", "") or ""
            self._debug(f"Initial valve aspect_ratio: '{_ar}'")
            self._debug(f"User prompt: {prompt[:200]}...")

            # If valve not provided, attempt to extract aspect ratio from the user's prompt
            if not _ar:
                try:
            ...
            # Try to apply aspect_ratio using ImageConfig per official docs
            if isinstance(_ar, str) and _ar.strip():
                ...
                if image_config_class:
                    try:
                        image_config = image_config_class(aspect_ratio=_ar.strip())
                        _cfg_kwargs["image_config"] = image_config
                        _cfg_kwargs["response_modalities"] = ["Image"]
                        self._debug(
                            f"✓ Applied ImageConfig with aspect_ratio={_ar.strip()}"
                        )
                    except Exception as e:
                    ...
                    self._debug(
                        f"⚠️ Aspect ratio '{_ar}' detected but SDK doesn't support ImageConfig"
                    )
            ...
            try:
                        ...
                        if inline and inline.data:
                            ...
                            try:
                                await self.emit_status(
                                    __event_emitter__,
                                    "info",
                                    f"Uploading generated image {image_count + 1}...",
                                )
                                ...
                                # Send image message via event emitter (like the working tool)
                ...
                if partial_text or images_block:
                    ...
                    body["messages"].append(
                        {
                            "role": "assistant",
                            "content": f"⚠️ Partial response before error:\n\n{combined}\n\n❌ Error: {str(e)}.{tip}",
                        }
                    )
                    return body
                else:
            ...
            self._debug(f"Returning body with {len(body['messages'])} messages")
...
{{MESSAGES:END:6}}"""
    ...
    def _extract_aspect_ratio_from_prompt(self, text: str) -> str:
        ...
        Supports google.genai SDK aspect ratios:
        - "1:1", "3:4", "4:3", "9:16", "16:9" (per ImageConfig docs)

        Patterns:
        - Numeric: '16:9', '9x16', '4/3', '16 x 9', etc.
        - Keywords: 'square'->1:1, 'portrait/vertical/tall'->9:16,
          'landscape/wide/cinematic'->16:9, 'classic'->4:3

        Returns normalized form like '16:9' or empty string if none.
        """
        # google.genai SDK supported aspect ratios per ImageConfig docs
    ...
    def _build_gemini_context(
        ...
        # If edit_mode, strongly prefer a single most recent image
        max_images = (
