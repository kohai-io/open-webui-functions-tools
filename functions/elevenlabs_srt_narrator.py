"""title: ElevenLabs Script Narrator (SRT & Drama)
author: open-webui
author_url: https://github.com/open-webui
funding_url: https://github.com/open-webui
version: 3.5
license: MIT
description: Convert SRT subtitles or drama scripts to synchronized speech using ElevenLabs TTS. Supports SRT format with timestamps and screenplay format with scene headers, character dialogue, and stage directions. Features automatic voice assignment per character, multi-voice narration, speaker detection, and precise duration tracking.
requirements: aiohttp, cryptography, pydantic, imageio-ffmpeg, pydub
...
class Pipe:
    class Valves(BaseModel):
        ...
        # Automatic voice assignment pool
        AUTO_VOICE_POOL: str = Field(
            default="21m00Tcm4TlvDq8ikWAM,pNInz6obpgDQGcFmaJgB,ErXwobaYiN019PkySvjV,MF3mGyEYCl7XYWbV9V6O,TxGEqnHWrfWFTfGW9XjX,VR6AewLTigWG4xSOukaG",
            description="Pool of voice IDs to auto-assign to characters (comma-separated). Rachel, Adam, Antoni, Elli, Josh, Arnold. Characters will be assigned voices in order of appearance.",
        )
        
        ENABLE_AUTO_VOICE_ASSIGNMENT: bool = Field(
            default=True,
            description="Auto-assign unique voices to each character from the voice pool (drama scripts only)",
        )
        ...
        NARRATOR_VOICE_ID: str = Field(
            default="",
            description="Voice ID for narrator (scene announcements & stage directions). Leave empty to use default voice. Recommended: onwK4e9ZLuTAKqWW03F9 (Clyde - authoritative), VR6AewLTigWG4xSOukaG (Arnold - documentary), pNInz6obpgDQGcFmaJgB (Adam - deep & clear).",
        )
    ...
    async def pipe(
        ...
        try:
            # Handle PDF files - use OWUI's native text extraction from file.data
            if script_filename.lower().endswith('.pdf'):
                # PDF text is already extracted and stored in file.data["content"] by OWUI
                if file_record.data and "content" in file_record.data:
                    script_content = file_record.data["content"]
                    self.log.info(f"Using pre-extracted PDF text from OWUI ({len(script_content)} chars)")
                else:
                    return "❌ **Error:** PDF text extraction failed or file has no text content.\n\n**Note:** Open WebUI extracts PDF text automatically when uploaded. Try re-uploading the PDF file."
            else:
        ...
        manual_voice_map = self._parse_speaker_voice_map()
        
        # Auto-assign voices to characters
        speaker_voice_map = self._auto_assign_voices(subtitles, manual_voice_map)
        ...
        for i, subtitle in enumerate(subtitles):
            ...
            if speaker == '__SCENE__':
                # Use narrator voice for scene announcements
                voice_id = self.valves.NARRATOR_VOICE_ID if self.valves.NARRATOR_VOICE_ID else self.valves.DEFAULT_VOICE_ID
            elif speaker == '__DIRECTION__':
                # Use narrator voice for stage directions
                voice_id = self.valves.NARRATOR_VOICE_ID if self.valves.NARRATOR_VOICE_ID else self.valves.DEFAULT_VOICE_ID
            else:
...
✨ The audio uses natural speech without pitch/speed changes."""
        ...
        if format_type == 'drama_script':
            # Build character voice assignments list
            voice_assignments = []
            manual_count = len(manual_voice_map)
            auto_count = len([s for s in unique_speakers if s in speaker_voice_map and s not in manual_voice_map])
            
            # Add narrator voice (for scenes and stage directions)
            narrator_voice_id = self.valves.NARRATOR_VOICE_ID if self.valves.NARRATOR_VOICE_ID else self.valves.DEFAULT_VOICE_ID
            narrator_elements = []
            if self.valves.SCENE_ANNOUNCEMENTS:
                narrator_elements.append("scenes")
            if self.valves.INCLUDE_STAGE_DIRECTIONS:
                narrator_elements.append("stage directions")
            
            if narrator_elements:
                narrator_desc = " + ".join(narrator_elements)
                voice_assignments.append(f"  - **🎙️ NARRATOR ({narrator_desc}):** `{narrator_voice_id}` 📖")
                voice_assignments.append("")  # Add blank line separator
            
            # Add character voices
            for speaker in sorted(unique_speakers):
                if speaker in speaker_voice_map:
                    voice_id = speaker_voice_map[speaker]
                    if speaker in manual_voice_map:
                        voice_assignments.append(f"  - **{speaker}:** `{voice_id}` 🎯 (manual)")
                    else:
                        voice_assignments.append(f"  - **{speaker}:** `{voice_id}` 🤖 (auto)")
                else:
                    voice_assignments.append(f"  - **{speaker}:** `{self.valves.DEFAULT_VOICE_ID}` (default)")
...
**Total Characters:** {len(unique_speakers)} | **Manual:** {manual_count} | **Auto:** {auto_count}

{chr(10).join(voice_assignments)}
"""
        else:
    ...
    def _parse_drama_script(self, script_content: str) -> List[Dict]:
        ...
        Extracts dialogue from screenplay format where dialogue can be:
        - Same line: "1 CHARACTER: text"
        - Next line: "1 CHARACTER:" followed by "text" on next line(s)
        ...
        while i < len(lines):
            ...
            # Check for numbered dialogue header: "8 DAWN:" or "8 DAWN: text"
            # Regex captures only the character name (letters), stripping the line number
            character_match = re.match(r'^\d+\s+([A-Z\s]+):\s*(.*)$', line)
            
            # Also try un-numbered format: "DAWN:" or "DAWN: text"
            if not character_match:
                character_match = re.match(r'^([A-Z\s]{2,}):\s*(.*)$', line)
            
            if character_match:
                # Extract clean character name (e.g., "8 DAWN:" → "DAWN")
                character = character_match.group(1).strip()
                text = character_match.group(2).strip()
                
                # If no text after colon, collect from following lines
                if not text:
                    dialogue_lines = []
                    i += 1
                    
                    # Collect dialogue lines until we hit another character, scene, or stage direction
                    while i < len(lines):
                        ...
                        # Stop if we hit another character line
                        if re.match(r'^\d+\s+[A-Z\s]+:', next_line) or re.match(r'^[A-Z\s]{2,}:', next_line):
                            break
                        
                        # Stop if we hit a scene header
                        if re.match(r'^(INT\.|EXT\.)', next_line):
                            break
                        
                        # Check if it's a stage direction (all caps, not parenthetical)
                        if next_line.isupper() and not next_line.startswith('('):
                            # Stage direction - handle separately if enabled
                            if self.valves.INCLUDE_STAGE_DIRECTIONS:
                                if dialogue_lines:
                                    # Save collected dialogue first
                                    break
                                else:
                                    # This is a stage direction for this character
                                    dialogue_lines.append(f"({next_line})")
                                    i += 1
                                    break
                            else:
                                i += 1
                ...
                # Skip if we ended up with no text
                if not text:
                    continue
                
                # Handle pure stage directions (in parentheses)
                ...
                # ~150 words per minute = 2.5 words/second = 400ms per word
            ...
            # Skip unmatched lines (likely stage directions or formatting)
            i += 1
    ...
    def _auto_assign_voices(self, subtitles: List[Dict], manual_voice_map: Dict[str, str]) -> Dict[str, str]:
        """Auto-assign unique voices to characters from the voice pool.
        
        Args:
            subtitles: List of parsed dialogue entries
            manual_voice_map: Manually configured speaker-to-voice mapping (takes precedence)
            
        Returns:
            Complete voice map with auto-assigned voices for unmapped characters
        """
        # Start with manual mappings
        voice_map = manual_voice_map.copy()
        
        # Check if auto-assignment is enabled
        if not self.valves.ENABLE_AUTO_VOICE_ASSIGNMENT:
            return voice_map
        
        # Parse voice pool
        voice_pool = [v.strip() for v in self.valves.AUTO_VOICE_POOL.split(',') if v.strip()]
        
        if not voice_pool:
            self.log.warning("Auto voice assignment enabled but voice pool is empty")
            return voice_map
        
        # Extract unique character names (excluding special speakers)
        characters = []
        seen_characters = set()
        ...
        for subtitle in subtitles:
            ...
            # Skip special speakers and already mapped characters
            if speaker in ['__SCENE__', '__DIRECTION__']:
                continue
            if speaker in manual_voice_map:
                continue
            if speaker in seen_characters:
                continue
            
            characters.append(speaker)
            seen_characters.add(speaker)
        
        # Assign voices from pool (cycle through if more characters than voices)
        for i, character in enumerate(characters):
            voice_id = voice_pool[i % len(voice_pool)]
            voice_map[character] = voice_id
            self.log.info(f"Auto-assigned '{character}' → voice {i % len(voice_pool) + 1}/{len(voice_pool)} ({voice_id[:15]}...)")
        
        if characters:
            self.log.info(f"Auto-assigned {len(characters)} character(s) to {len(voice_pool)} voice(s)")
