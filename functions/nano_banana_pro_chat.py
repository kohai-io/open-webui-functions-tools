...
requirements: google-genai>=1.50.0, cryptography, requests, google-auth
"""
...
class Pipe:
    ...
    def __init__(self):
        ...
        if self.valves.model_name == "gemini-2.5-flash-image":
            ...
            migrations.append("model_name: gemini-2.5-flash-image → gemini-3-pro-image-preview")
        
        if migrations:
            print(f"[nano_banana_chat] Auto-migrated config: {', '.join(migrations)}")

    async def on_startup(self):
        print(f"[nano_banana_chat] Pipe loaded successfully - version 2.6 (Gemini 3 Pro Image support)")
        print(f"[nano_banana_chat] Pipe type: Default (inline display)")
        print(f"[nano_banana_chat] Features: 1K/2K/4K resolution, 10 aspect ratios, up to 14 reference images")
        print(f"[nano_banana_chat] Sticky settings: History checked BEFORE prompt to avoid false positives")
        print(f"[nano_banana_chat] Debug mode: {self.valves.debug}")
        print(f"[nano_banana_chat] Auth mode: {'Vertex AI' if self.valves.use_vertex_ai else 'API Key'}")
    ...
    async def pipe(
        ...
        try:
            ...
            if not _ar or not _res:
                try:
                    sticky_ar, sticky_res = self._extract_sticky_settings_from_history(messages)
            ...
            if (isinstance(_ar, str) and _ar.strip()) or (isinstance(_res, str) and _res.strip()):
                ...
                if image_config_class:
                    try:
                        ...
                        config_desc = ", ".join([f"{k}={v}" for k, v in image_config_kwargs.items()])
                        self._debug(
                            f"✓ Applied ImageConfig with {config_desc}"
                        )
                    except Exception as e:
                    ...
                    for i, part in enumerate(parts):
                        ...
                        if inline and inline.data:
                            ...
                            try:
                                ...
                                # Don't emit message here - will return it at the end for inline display
                            except Exception as e:
            ...
            self._debug(f"Appended assistant message with {len(response_content)} chars")
...
{{MESSAGES:END:6}}"""
    ...
    def _extract_sticky_settings_from_history(self, messages: List[Dict]) -> tuple[str, str]:
    ...
    def _build_gemini_context(
        ...
        self._debug(f"Image collection: edit_mode={self.valves.edit_mode}, max_images={max_images}")
